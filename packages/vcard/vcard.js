...
/*
	We change the delimiters to look like handlebars
 */
_.templateSettings = {
  interpolate: /\{\{(.+?)\}\}/g
};
...
function parseContactDetails(contact){
	return _.template(DETAILS)(contact);
}

function parseContactPhones(contact){
	var result = '';
	if (_.isArray(contact.phones) && contact.phones.length > 0)
		result += _.template(WORKVOICE)({phone: contact.phones[0]});
	if (_.isArray(contact.phones) && contact.phones.length > 1)
		result += _.template(HOMEVOICE)({phone: contact.phones[1]});
	if (_.isArray(contact.phones) && contact.phones.length > 2)
		result += _.template(CELLVOICE)({phone: contact.phones[2]});
	return result;
}

function parseContactAddesses(contact){
	var baseAddress = {street: '', city: '', dep: ''};
	var result = '';
	if (_.isArray(contact.addresses) && contact.addresses.length > 0)
		result += _.template(WORKADDRESS)(_.extend(baseAddress, contact.addresses[0]));
	if (_.isArray(contact.addresses) && contact.addresses.length > 1)
		result += _.template(HOMEADDRESS)(_.extend(baseAddress, contact.addresses[1]));
	if (_.isArray(contact.addresses) && contact.addresses.length > 2)
		result += _.template(POSTALADDRESS)(_.extend(baseAddress, contact.addresses[2]));
	return result;
}
...
function parseContactEmails(contact){
	var result = '';
	if (_.isArray(contact.emails) && contact.emails.length > 0)
		result += _.template(PREFEMAIL)({email: contact.emails[0]});
	if (_.isArray(contact.emails) && contact.emails.length > 1)
		result += _.template(SECEMAIL)({email: contact.emails[1]});
	if (_.isArray(contact.emails) && contact.emails.length > 2)
		result += _.template(THIRDEMAIL)({email: contact.emails[2]});
	return result;
}
